...
function Conversation({ conversationName }) {
  ...
  useEffect(() => {
    const socket = io('https://chatapp.review24info.online/', {
    ...
    const fetchMessages = async () => {
      try {
        const response = await fetch(`https://chatapp.review24info.online/messages/${activeConversationName}`);
        if (response.ok) {
  ...
  const leaveConversation = async () => {
    try {
      const response = await fetch(`https://chatapp.review24info.online/leaveconversation/${activeConversationName}/${activeUser}`, {
        method: 'DELETE',
        headers: {
